...
#use frame directory
    ...
    def evict_page(self):
        ...
        if(evict_frame.dirtyBit == True):
            ...
            if self.frame_info[evict_index][2] == 'b':
                    ...
                    self.write_numRecords(path, frame_size + 10, evict_index)
                    ...
                    self.write_numRecords(path, frame_size + 13, evict_index)
    ...
    def get_empty_frame(self, numColumns):
        if not self.has_capacity():
            ...
            self.frames[frame_index] = Frame(None, numColumns)
            # Initialize frame data
            for i in range(numColumns):
                self.frames[frame_index].frameData[i] = Page()
        else:
            frame_index = self.numFrames
            self.frames.append(Frame(None, numColumns))
            # Initialize frame data
            for i in range(numColumns):
                self.frames[self.numFrames].frameData[i] = Page()
            self.numFrames += 1
        return frame_index
...
class Frame:
    ...
    def __init__(self, path, numColumns):
        ...
        self.lastAccess = datetime.now()  # Initialize with current time
