...
class Table:
    ...
    def updateRec(self, current_rid, *columns):
        ...
        if origin_rid is None:
            origin_rid = current_rid

        # Save original data
        origin_columns = []
        for j in range(len(columns)):
            value = base_frame.read_data(j, record_id)
            origin_columns.append(value)
        ...
        # Prepare to insert new tail record
        numTPS = 0
        ...
        if self.tail_page_frame_index is None:
            return False
        ...
        for j in range(len(columns)):
            if columns[j] is not None:
                tail_frame.write_data(j, origin_columns[j])
                schema += '1'
            else:
                tail_frame.write_data(j, None)
                schema += '0'

        # Update tail page metadata
        tail_frame.schema_encoding.append(schema)
        tail_frame.numRecords += 1
        tail_frame.set_indirection(tail_frame.numRecords - 1, current_rid)
        tail_frame.BaseRID.append(origin_rid)
        tail_frame.rid.append(new_rid)

        # Update base page metadata
        base_frame.set_indirection(record_id, new_rid)
    ...
    def get_record(self, rid):
        ...
        if all(col is None for col in record_columns):  # If all columns are None, return None
            frame.unpin_page()
            return None
    ...
    def get_record_version(self, rid, version):
        ...
        # Get original RID
        version_rid = base_frame.get_indirection(record_id)
        if version_rid is None or version_rid == rid:
            return None

        # Save original data
        version_records = []
        current_rid = version_rid

        # Iterate through version chain
        while current_rid is not None and current_rid != rid:
            version_page_range_index, version_page_index, version_record_id, version_mark = current_rid
            frame_index = self.bufferpool.load_page((version_page_range_index, version_page_index, version_record_id, version_mark))
            ...
            if record_columns and not all(col is None for col in record_columns):
                version_records.append((current_rid, record_columns))
        ...
        if version == -1:  # last version
            if version_records:
                last_rid, last_columns = version_records[-1]
                return Record(last_rid, last_columns[self.key], last_columns)
        else:  # specific version
            target_version = abs(version)
            if 0 <= target_version - 1 < len(version_records):
                target_rid, target_columns = version_records[target_version - 1]
                return Record(target_rid, target_columns[self.key], target_columns)

        # If version is not found, return None
        record_columns = self.bufferpool.extract_data(base_frame_index, self.num_columns, record_id)
        if all(col is None for col in record_columns):  # If all columns are None, return None
            return None
        record_key = record_columns[self.key]
